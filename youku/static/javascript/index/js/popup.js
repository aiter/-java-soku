...
Popup.prototype={
	iframeIdName:'ifr_popup',initialize:function (config){
		...
		this.info={shadowWidth:4,title:"",contentUrl:"",contentHtml:"",callBack:null,parameter:null,confirmCon:"",alertCon:"",someHiddenTag:"select,object,embed",someHiddenEle:"",overlay:0,coverOpacity:60,framename:'ifr_popup'};
	...
	build:function (){
		...
		var html='<div id="dialogBox" style="z-index:'+showZIndex+';position:absolute;top:'+p[0]+'px;left:'+p[1]+'px;width:'+this.config.width+'px;height:'+this.config.height+'px;">\
				<div style="position:relative;z-index:'+showZIndex2+';top:0;left:0;width:'+this.config.width+'px;background:#fff;border:1px solid #9dccf6;filter : progid:DXImageTransform.Microsoft.DropShadow(color=#ffc3c3c3,offX=4,offY=4,positives=true);">\
					<div style="position:relative;height:20px;padding:10px;overflow:hidden;font-size:14px;font-weight:bold;">'+this.info.title+'<div style="cursor:pointer;position:absolute;right:10px;top:11px;width:18px;height:18px;overflow:hiddden;" id="dialogBoxClose"><div class="ico__winclose" title="关闭"></div></div></div>\
						<iframe scrolling="no" name="'+this.info.framename+'" style="width:100%;height:'+(this.config.height-22)+'px;" frameborder="0" src="'+this.info.contentUrl+'"></iframe>\
					</div>\
			</div>';
		          
		html+='<div id="dialogBoxShadow" style="position:absolute;z-index:'+baseZIndex+';top:'+(p[0]+4)+'px;left:'+(p[1]+4)+'px;width:'+(this.config.width+2)+'px;height:'+(this.config.height+22)+'px;background:#c3c3c3;">fefewewf</div>';
