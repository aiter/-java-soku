...
public class ReverseLatestListener  extends TimerTask{
	private static Log log = LogFactory.getLog(ReverseLatestListener.class);
	...
	public void run() {
		if (!isRunning) {
			...
			System.out.println(new Date()+"--start--根据file_id将当日上传的相关剧集添加至episode_video,start,总内存:" + Runtime.getRuntime().totalMemory()+",剩余内存:"+Runtime.getRuntime().freeMemory());
			
			try {
				List<Episode> episodes=EpisodeMgt.getInstance().getLastUploadEpisodes();
				...
				for(Episode episode:episodes){
					...
					for(Video v:videos){
						flag=EpisodeVideoMgt.getInstance().addEpisodeVideo(episode.getId(), v);
						if(false==flag)break;
						System.out.println("最新上传,addEpisodeVideo:eid:"+episode.getId()+",vid:"+v.getVid());
						Thread.sleep(35);
					}
				}
			} catch(Exception e){
			...
			System.out.println(new Date()+"--end--根据file_id将当日上传的相关剧集添加至episode_video,end,总内存:" + Runtime.getRuntime().totalMemory()+",剩余内存:"+Runtime.getRuntime().freeMemory()+",占用时间:"+(System.currentTimeMillis()-s));
			isRunning = false;
		} else {
