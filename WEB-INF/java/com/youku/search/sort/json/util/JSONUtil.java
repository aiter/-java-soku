package com.youku.search.sort.json.util;
...
public class JSONUtil {
	...
	public static String array2String(JSONArray array,String separator,String defaultValue){
		...
		for (int i = 0; i < array.length(); i++) {
			Object tmp = array.optString(i);
			if(tmp instanceof JSONObject){
				sBuilder.append(((JSONObject)tmp).optString("name")).append(separator);
			}else {
				sBuilder.append(tmp).append(separator);
			}
		}
	...
	public static JSONArray getPerson(JSONArray array, String personType) {
		if(isEmpty(array)){
			return null;
		}
		...
		for (int i = 0; i < array.length(); i++) {
			...
			if(object instanceof JSONObject){
				...
				if (personType.equals(tmp.optString("type"))){
					persons.put(tmp.optString("name"));
				}
				...
				if (person_type.equals(personType)) {
					persons.put(person_name);
				}
			}
		}
	...
	public static String joinPerson(JSONArray array, String separator, int count,String personType) {
		...
		JSONArray persons = getPerson(array, personType);

		if(!isEmpty(persons)){
			return join(persons, separator,count);
		}
	...
	public static String joinPerson(JSONArray array, String separator, int count,String personType,String defaultValue) {
		String tmp = joinPerson(array, separator, count, personType);
