...
public class ServerManager {
			...
			if (prop != null && groupCount >0)
			{
				for (int i = 0;i<groupCount;i++)
				{
					...
					if (servers != null && servers.size() > 0)
					{
						...
						groupMap.put("log_" + (i+1),statList);
	...
	public static List<Server> getLogServers(int groupNumber)
	{
		return groupMap.get("log_" + groupNumber);
	}
	public static List<Server> getLogServers()
	{
		List<Server> list = new ArrayList<Server>();
		for (int i=1;i<=groupCount;i++)
		{
			list.addAll(groupMap.get("log_"+i));
		}
		return list;
	}
	...
	{
		return config.getString("indexpath.library");
	}
	...
	{
		return config.getString("indexpath.log");
	}
	public static String getLogQuickIndexPath()
	{
		return config.getString("indexpath.logquick");
	}
	...
	public static Server getServer(int group,int type,int order)
	{
		...
		else if (type == Constant.QueryField.LOG
				|| type == Constant.QueryField.LOGLIKE
				|| type == Constant.QueryField.QUICK){
			servers = getLogServers(group);
		}
		else
			return null;
		for (int j= 0;j<servers.size();j++)
