...
public class ManageUserAuthenticationInterceptor implements Interceptor {
        ...
        if (!isAuthenticated) {
        	...
        	if (user.getIsSuperUser() != 1) {
				...
				AuthPermission manageItemPermission = permissionMap
						.get(Constants.MANAGE_USER_PERMISSION);
				
				AuthPermission manageShieldPermission = permissionMap
				...
				try {
					...
					for (UserPermission userPermission : permissionsList) {
						...
						if (manageShieldPermission != null && userPermission.getPermissionId() == manageShieldPermission
								.getId()) {
							isPermit = true;
							break;
						}
