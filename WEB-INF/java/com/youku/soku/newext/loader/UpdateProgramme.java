...
import com.youku.soku.newext.redis.ProgrammeCache;
import com.youku.soku.newext.util.MiscUtil;
import com.youku.soku.newext.util.SortUtil;


public class UpdateProgramme {
	private static Log logger = LogFactory.getLog(UpdateProgramme.class);

	public static void doUpdate(Programme programme, AliasInfo teleplayInfo,
		...
		for (String proName : programmeNameSet) {
			List<Programme> programmeList = teleplayInfo.getName_programme()
					.get(proName);
		...
		ProgrammeCache cache = new ProgrammeCache();
		cache.cacheProgrammeJson(programme.getName(), programme.getCate());

	}

	// 将某个programme 对应的List<ProgrammeSite>加入到 name_programme
	private static void load(Programme programme, AliasInfo info,
		...
		for (String proName : programmeNameSet) {
			MiscUtil.putIfAbsent(info.getName_programme(), StringUtils
					.trimToEmpty(proName), new ArrayList<Programme>());

			if (!info.getName_programme().get(StringUtils.trimToEmpty(proName))
					.contains(programme)) {
				info.getName_programme().get(StringUtils.trimToEmpty(proName)).add(
						programme);
