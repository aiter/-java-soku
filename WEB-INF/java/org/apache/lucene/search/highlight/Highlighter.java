...
public class Highlighter
{
	...
	private boolean isEnd =  true;//截取是否到最后，如果没有到最后加separator
		...
		throws IOException
	{
		...
		String r = getBestFragment(tokenStream, text);
		tokenStream.close();
		return r;
	}
		...
		throws IOException
	{
		...
		String[] r = getBestFragments(tokenStream, text, maxNumFragments);
		tokenStream.close();
		return r;
	}
	/**
		...
		throws IOException
	{
		...
		try
		{
			...
			while (token!= null)
			{
//				System.out.println(token.termText()+"\ttokenGroup.numTokens="+tokenGroup.numTokens+"\ttokenGroup.endoffset="+tokenGroup.endOffset +"\ttoken.startOffset()="+token.startOffset() + "\tendOffset="+token.endOffset());
...
//				}
				...
				if (token.endOffset() >= text.length())//如果到达末尾则结束循环，不管后面是否还有token，可能后面还有第几集 之类的token，但由于提前处理，所以后面省略
					break;
				token = tokenStream.next();
			}
			...
			if(tokenGroup.numTokens>0)
			{
				...
				if (startOffset >= text.length() || endOffset > text.length()){
					System.err.println("out array:" + text);
				}
				else{
					tokenText = text.substring(startOffset, endOffset);
					String markedUpText=formatter.highlightTerm(encoder.encodeText(tokenText), tokenGroup);
...
//			System.out.println("lastEndOffset="+lastEndOffset + "\ttext.length()="+text.length() + "\tmaxDocBytesToAnalyze=" + maxDocBytesToAnalyze);
...
//				System.out.println(currentFrag.toString() + currentFrag.textStartPos + "   " +currentFrag.textEndPos + "    "+currentFrag.score);
				fragQueue.insert(currentFrag);
			}
			//return the most relevant fragments
			TextFragment frag[] = new TextFragment[fragQueue.size()];
//			System.out.println("fragQueue.size()="+fragQueue.size());
