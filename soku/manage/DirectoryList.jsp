	...
	$(document).ready(function() {
		...
		function showResponse(responseText, statusText)  { 
	       ...
	       // $('#TopWord_save_topWord_picturePath').attr("disabled","disabled");
	        $.ajax({
				url: "<s:url action="Directory_updatePicAjax" />",
				type: "POST",
				data: "directoryInfoId=" + id + "&picPath=" + o.filePath  + "&startDate=" + $('#startDate').val(),
		...
		$("#sorttable").tableDnD({
		     ...
		     //   alert(debugStr);
			     ...
			     $.ajax({
						url: "<s:url action="Directory_updateOrder" />",
						type: "POST",
	...
	function cancelupdateorder(){		    
		...
		$(function() {
			...
			$("#activeTopDate").click(function () {
				...
				var channelName = $("#Directory_list_channelName").val();
				...
				$.ajax({					
					url: '<s:url action="Directory_changeTopDate" />',
					type: 'GET',
			...
			$('#downloadFile').click(function () {
				...
				$("#Directory_list").attr("action", "Directory_exportFile.do");
				$("#Directory_list").submit();
				$("#Directory_list").attr("action", "Directory_list.do");
			...
			$("#Directory_list_channelName").change(function() {
				if($("#Directory_list_channelName").val() == '10') {
					var url = '<s:url action="DirectoryPerson_list" />';
					location.href = url + '?channelName=' + $("#Directory_list_channelName").val() + '&startDate=' + $("#startDate").val();
					return;
				}
				$("#TopWord_list").submit();
			});
			...
			$("a[name='deleteLink11']").click(function() {
				...
				$.ajax({					
					url: '<s:url action="Directory_deleteAjax" />',
					type: 'GET',
					data: 'directoryInfoId=' + id,
		...
		function deleteAjax(id) {
			...
			$.ajax({					
				url: '<s:url action="Directory_deleteAjax" />',
				type: 'GET',
				data: 'directoryInfoId=' + id + "&startDate=" + $('#startDate').val(),
		...
		function readdAjax(id) {
			...
			$.ajax({					
				url: '<s:url action="Directory_reAddAjax" />',
				type: 'GET',
				data: 'directoryInfoId=' + id + "&startDate=" + $('#startDate').val(),
		...
		function deleteForEverAjax(id) {
			...
			$.ajax({					
				url: '<s:url action="Directory_deleteForEverAjax" />',
				type: 'GET',
				data: 'directoryInfoId=' + id,
		...
		function submitchangechannelname(id) {
			...
			$.ajax({					
				url: '<s:url action="Directory_updateAjax" />',
				type: 'GET',
				data: 'directoryInfoId=' + id + '&channelName=' + $('#channelNameValue' +id).val(),
...
<div id="main">
  ...
  <div id="middle">
    ...
    <div id="center-column">
      <div class="top-bar"> 
      	<a href="<s:url action="TopWord_input">
			<s:param name="directoryInfoId" value="-1" />
      ...
      <div class="select-bar">
         <s:form action="Directory_list" validate="false" cssClass="form" theme="simple">
	        <s:select list="channelList" listKey="id" listValue="label" key="channelName"></s:select>
      		 <s:textfield id="startDate" name="startDate" />
      		  <s:select key="visible" list="#{-1:'所有', 1:'屏蔽', 0:'正常'}" listKey="key" listValue="value"></s:select>
	        <label>
	        ...
	        <input type="button" id="activeTopDate" class="ui-button ui-state-default ui-corner-all" value="上线" />
	        </s:if>
	        
	         <input type="button" id="downloadFile" class="ui-button ui-state-default ui-corner-all" value="导出数据" />
      	</s:form> 
      	
      
      	
      	
      	<s:form action="TopWord_changeTopDate" id="changetd">
      ...
      <div class="table"> <img src="img/bg-th-left.gif" width="8" height="7" alt="" class="left" /> <img src="img/bg-th-right.gif" width="7" height="7" alt="" class="right" />
        <s:form action="TopWord" validate="false" cssClass="form" theme="simple">
			...
			<tbody>
				...
				<s:iterator value="pageInfo.results" status="index">
					<tr id="<s:property value="id" />">
						<td align="center" class="editOrder, dragHandle">
						...
						<td align="left">
							<s:if test="fk_cate_id == 1">
								<a href="/TeleplayVersion_input.do?teleplayVersionId=<s:property value="version_id"/>">
									<s:property value="keyword"/>						
								</a>
							</s:if>
							<s:if test="fk_cate_id == 2">
								<a href="/Movie_input.do?movieId=<s:property value="version_id"/>">
									<s:property value="keyword"/>						
								</a>
							</s:if>
							<s:if test="fk_cate_id == 3">
								<a href="/Variety_input.do?varietyId=<s:property value="version_id"/>">
									<s:property value="keyword"/>						
								</a>
							</s:if>
						...
						<td align="center"><s:property value="cates" /> </td>
						...
						<td align="center"><s:property value="union_searchs" /></td>
						
				
					<td align="center">
						
							<s:if test="#session.user_permission.block_top_word != null">
								<a id="deleteLink<s:property value="id" />"
							 onclick="deleteAjax(<s:property value="id" />);return false" href="#" class="listbutton<s:if test='visible == 0'>, highlight</s:if>">
								永久屏蔽 </a> &nbsp;<br />
							
							
								<a id="readdLink<s:property value="id" />"
								 onclick="readdAjax(<s:property value="id" />);return false" href="#" class="listbutton<s:if test='visible == 1'></s:if>">
								解除屏蔽 </a> &nbsp;<br />
								
								<s:if test="fk_cate_id == 1">
								<a class="listbutton" href="/TeleplayVersion_input.do?teleplayVersionId=<s:property value="version_id"/>">
									修改					
								</a>
							</s:if>
							<s:if test="fk_cate_id == 2">
								<a class="listbutton" href="/Movie_input.do?movieId=<s:property value="version_id"/>">
									修改						
								</a>
							</s:if>
							<s:if test="fk_cate_id == 3">
								<a class="listbutton" href="/Variety_input.do?varietyId=<s:property value="version_id"/>">
									修改						
								</a>
							</s:if>
							</s:if>
				 ...
				 <tfoot>
					<tr>
						...
						<s:if test="pageInfo.hasPrevPage">
							<a href="<s:url action="Directory_list"> 
        ...
        <div class="select"> <strong>Other Pages: </strong>
           <input type="hidden" id="pageurl" value="<s:url action="Directory_list">          						
								<s:param name="channelName" value="channelName" />
								<s:param name="startDate" value="startDate" />
								<s:param name="counter" value="counter" />
