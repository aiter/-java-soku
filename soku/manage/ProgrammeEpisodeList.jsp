...
<html>
<head>
<title><s:text name="programmeEpisode.title.list" /></title>
	...
	<script type="text/javascript"><!--
		$(function() {
			...
			$("#saveEpisode").click(function() {
				alert('<s:url action="ProgrammeEpisode_save" />');
			});
		...
		function saveEpisode(orderId) {
			$("#savesuccess"+ orderId).hide();
			$("#savefail" + orderId).hide();
			var url = $("#url" + orderId).val();
			
			var programmeSiteId= '<s:property value="programmeSiteId"/>';
			...
			var orderStage = $("#orderStage" + orderId).val();
			var hd = $("#hd" + orderId).attr("checked") ? 1 : 0;
			$.ajax({
				url: '<s:url action="ProgrammeEpisode_save" />',
				type: 'POST',
				data: 'programmeSiteId=' + programmeSiteId + '&sourceSiteId=' + sourceSiteId + '&url=' + url + '&id=' + id + '&orderId=' + orderId + '&orderStage=' + orderStage + '&logoUrl=' + logoUrl + '&seconds=' + seconds + '&hd=' + hd,
		...
		function getVideoDetail(orderId) {
			...
			$.ajax({
				...
				type: 'POST',
				data: 'programmeSiteId=' + programmeSiteId + '&sourceSiteId=' + sourceSiteId + '&url=' + url,
				success: function(data) {
					var o = eval('(' + data + ')');	
		...
		function addToBlacklist(orderId) {
			var id = $("#hiddenId" + orderId).val();
			$.ajax({
				url: '<s:url action="ProgrammeEpisode_addToBlacklist" />',
				type: 'POST',
				data: 'id=' + id,
				success: function(data) {
					var o = eval('(' + data + ')');
		...
		function changeLockStatus(orderId) {
			var id = $("#hiddenId" + orderId).val();
			$.ajax({
				url: '<s:url action="ProgrammeEpisode_changeLockStatus" />',
				type: 'POST',
				data: 'id=' + id,
				success: function(data) {
					var o = eval('(' + data + ')');
		...
		function searchEpisode(orderId) {
			...
			var id = $("#hiddenId" + orderId).val();
			$.ajax({
				url: '<s:url action="ProgrammeEpisode_searchProgrammeAjax" />',
				type: 'POST',
				data: 'programmeSiteId=' + programmeSiteId + '&sourceSiteId=' + sourceSiteId + '&orderId=' + orderId,
				success: function(data) {
					if(data == '[]') {
						...
						var keyword = $("#ProgrammeEpisode_searchProgramme_programmeNames").val();
		...
		function saveAllEpisode() {
			...
			$("#ProgrammeEpisode_searchProgramme").submit();
		}
...
<div id="main">
  ...
  <div id="middle">
    ...
    <div id="center-column">
      <div class="top-bar"> 
        ...
        <h1> <span class="chevron"> » </span> <s:text name="soku.programmeEpisode.title" /></h1>
      </div>
      <br />
      ...
      <div class="table"> <img src="img/bg-th-left.gif" width="8" height="7" alt="" class="left" /> <img src="img/bg-th-right.gif" width="7" height="7" alt="" class="right" />
        <s:form action="ProgrammeEpisode_list" validate="false" cssClass="form" theme="simple">
        <table class="listing" cellpadding="0" cellspacing="0">

				<tr>
					<th align="center" colspan="6"><s:property value="programmeNames" /><s:hidden name="programmeNames" /></th>
				</tr>

				<tr>
					
					<td colspan="6">	
							<s:hidden name="programmeSiteId"></s:hidden>		
				...
				<s:iterator value="pageInfo.results" status="index">
					<tr <s:if test="#index.odd">class="bg"</s:if>>
						...
						<td align="left" width="25%"><input id="url<s:property value="orderId"/>" name="pageInfo.result[<s:property value="orderId"/>].url" value="<s:property value="url"/>" class="text" 
						...
						<td width="20%">
							<div>
								<img src='<s:property value="logo" />' id="logoImg<s:property value="orderId"/>" /> <br/>								
							 	<input type="hidden" id="logoUrl<s:property value="orderId"/>" value="<s:property value="logo" />" name="pageInfo.result[<s:property value="orderId"/>].logo"/>
								<input id="changelogo<s:property value="orderId"/>" type="button" onclick='changeLogo(<s:property value="orderId"/> );' 
										 				value="更改图片" />	<br />
								集数 期数：<input id="orderStage<s:property value="orderId"/>" name="orderStage" type="text" value="<s:property value="orderStage"/>" class="editing" /> <br/>
								时长：<input id="seconds<s:property value="orderId"/>" name="seconds" type="text" value="<s:property value="seconds"/>" class="editing"  onchange="sethiddenseconds(<s:property value="orderId"/>)"/> <br/>
									 <input id="secondsH<s:property value="orderId"/>" type="hidden" value="<s:property value="seconds"/>" class="editing"  name="pageInfo.result[<s:property value="orderId"/>].seconds"/>
								...
								<input type="hidden" id="hdH<s:property value="orderId"/>" value="<s:property value="hd"/>"  name="pageInfo.result[<s:property value="orderId"/>].hd"/>
							</div>
						</td>
						<td align="left" width="5%">
										  <input id="savebtn<s:property value="orderId"/>" type="button" onclick='saveEpisode(<s:property value="orderId"/>);' value="保存"></input>
										 <input id="hiddenId<s:property value="orderId"/>" name="pageInfo.result[<s:property value="orderId"/>].id" type="hidden" value="<s:property value="id"/>"/>
										 <input name="pageInfo.result[<s:property value="orderId"/>].orderId" type="hidden" value="<s:property value="orderId"/>"/>
										 <input id="lock<s:property value="orderId"/>" type="button" onclick='addToBlacklist(<s:property value="orderId"/> );' 
										 				value="黑名单"
										 				/>
										 <div>
				...
				<td align="left" width="5%" colspan="6">
				 ...
				 <tfoot>
					<tr>
						...
						<a href="<s:url action="ProgrammeEpisode_list">
								<s:param name="pageNumber" value="pageInfo.currentPageNumber + 1" />
								<s:param name="programmeSiteId"><s:property value="programmeSiteId"/></s:param>
							</s:url>"><s:text
								name="nextpage"/></a>
						</s:if></td>
					</tr>
  ...
  <div id="dialog" title="更改收录集数">
	...
	<s:form action="ProgrammeEpisode_updateEpisodeCollected" validate="false" cssClass="form" enctype="multipart/form-data">
   ...
   <div id="dialogSite" title="选择站点">
	...
	<s:form action="ProgrammeEpisode_list" validate="false" cssClass="form" enctype="multipart/form-data" theme="simple">
