	...
	<script type="text/javascript"><!--
		...
		function saveEpisode(orderId) {
			...
			$.ajax({
				...
				type: 'POST',
				data: 'animeSiteVersionId=' + animeSiteVersionId + '&sourceSiteId=' + sourceSiteId + '&url=' + url + '&id=' + id + '&orderId=' + orderId + '&logoUrl=' + logoUrl + '&seconds=' + seconds + '&hd=' + hd,
		...
		function saveAllEpisode() {
			...
			$("#AnimeEpisode_searchAnime").submit();
		}
		...
		function getVideoDetail(orderId) {
			...
			var hd = $("#hd" + orderId).attr("checked") ? 1 : 0;
			$.ajax({
				url: '<s:url action="AnimeEpisode_searchAnimeDetail" />',
				type: 'POST',
				data: 'animeSiteVersionId=' + animeSiteVersionId + '&sourceSiteId=' + sourceSiteId + '&url=' + url,
				success: function(data) {
					var o = eval('(' + data + ')');	
...
<div id="main">
    ...
    <div id="center-column">
      ...
      <div class="table"> <img src="img/bg-th-left.gif" width="8" height="7" alt="" class="left" /> <img src="img/bg-th-right.gif" width="7" height="7" alt="" class="right" />
        ...
        <table class="listing" cellpadding="0" cellspacing="0">
				...
				<tr>
					<th align="center" colspan="7"><s:property value="animeNames" /><s:hidden name="animeNames" /></th>
				</tr>
				...
				<s:iterator value="animeEpisodeList" status="index">
					<tr <s:if test="#index.odd">class="bg"</s:if>>
						...
						<td align="left" width="25%"><input id="url<s:property value="orderId"/>" name="animeEpisodeList[<s:property value="orderId"/>].url" value="<s:property value="url"/>" class="text" 
						...
						<td width="20%">
							<div>
								<img src='<s:property value="logo" />' id="logoImg<s:property value="orderId"/>" /> <br/>								
							 	<input type="hidden" id="logoUrl<s:property value="orderId"/>" value="<s:property value="logo" />" name="animeEpisodeList[<s:property value="orderId"/>].logo"/>
								...
								时长：<input id="seconds<s:property value="orderId"/>" name="seconds" type="text" value="<s:property value="seconds"/>" class="editing"  onchange="sethiddenseconds(<s:property value="orderId"/>)"/> <br/>
									 <input id="secondsH<s:property value="orderId"/>" type="hidden" value="<s:property value="seconds"/>" class="editing"  name="animeEpisodeList[<s:property value="orderId"/>].seconds"/>
								...
								<input type="hidden" id="hdH<s:property value="orderId"/>" value="<s:property value="hd"/>"  name="animeEpisodeList[<s:property value="orderId"/>].hd"/>
							</div>
						</td>
						...
						<td align="left" width="5%"><input id="savebtn<s:property value="orderId"/>" type="button" onclick='saveEpisode(<s:property value="orderId"/>);' value="保存"></input>
										 <input id="hiddenId<s:property value="orderId"/>" name="animeEpisodeList[<s:property value="orderId"/>].id" type="hidden" value="<s:property value="id"/>"/>
										 <input name="animeEpisodeList[<s:property value="orderId"/>].orderId" type="hidden" value="<s:property value="orderId"/>"/>
  ...
  <div id="dialog" title="更改收录集数">
   ...
   <div id="dialogSite" title="选择站点">
	...
	<s:form action="AnimeEpisode_list" validate="false" cssClass="form" enctype="multipart/form-data" theme="simple">
