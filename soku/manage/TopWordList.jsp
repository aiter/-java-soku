	...
	$(document).ready(function() {
		...
		function showResponse(responseText, statusText)  { 
	       ...
	       // $('#TopWord_save_topWord_picturePath').attr("disabled","disabled");
	        $.ajax({
				url: "<s:url action="TopWord_updatePicAjax" />",
				type: "POST",
				data: "topWordId=" + id + "&picPath=" + o.filePath,
		...
		$("#sorttable").tableDnD({
		     ...
		     //   alert(debugStr);
			     $.ajax({
						url: "<s:url action="TopWord_updateOrder" />",
						type: "POST",
						data: "newOrder=" + newOrder + "&orginalOrder=" + $('#TopWord_orginalOrder').val(),
						success: function(data) {
								var jsonArr = eval('(' + data + ')');
								if(jsonArr.status == "success") {
	...
	function cancelupdateorder(){		    
		...
		$(function() {
			...
			$("#activeTopDate").click(function () {
				...
				var channelName = $("#TopWord_list_channelName").val();
				...
				$.ajax({					
					url: '<s:url action="TopWord_changeTopDate" />',
					type: 'GET',
			...
			$("a[name='deleteLink11']").click(function() {
				...
				$.ajax({					
					url: '<s:url action="TopWord_deleteAjax" />',
					type: 'GET',
					data: 'topWordId=' + id,
		...
		function readdAjax(id) {
			...
			$.ajax({					
				url: '<s:url action="TopWord_reAddAjax" />',
				type: 'GET',
				data: 'topWordId=' + id,
		...
		function deleteForEverAjax(id) {
			...
			$.ajax({					
				url: '<s:url action="TopWord_deleteForEverAjax" />',
				type: 'GET',
				data: 'topWordId=' + id,
		...
		function submitchangechannelname(id) {
			...
			$.ajax({					
				url: '<s:url action="TopWord_updateAjax" />',
				type: 'GET',
				data: 'topWordId=' + id + '&channelName=' + $('#channelNameValue' +id).val(),
    ...
    <div id="center-column">
      <div class="top-bar"> 
      	<a href="<s:url action="TopWord_input">
			<s:param name="topWordId" value="-1" />
      ...
      <div class="select-bar">
         <s:form action="TopWord_list" validate="false" cssClass="form" theme="simple">
	        <s:select list="channelList" listKey="name" listValue="label" key="channelName"></s:select>
      ...
      <div class="table"> <img src="img/bg-th-left.gif" width="8" height="7" alt="" class="left" /> <img src="img/bg-th-right.gif" width="7" height="7" alt="" class="right" />
        ...
        <table id="sorttable" class="listing" cellpadding="0" cellspacing="0">
			<tbody>
				<tr>
					...
					<th align="center" width="5%">直达区</th>
				...
				<s:iterator value="pageInfo.results" status="index">
					<tr id="<s:property value="id" />">
						...
						<td align="center" id="channelName<s:property value="id" />" ondblclick="changechannelname('<s:property value="id" />');">
							<div id="channelNameS<s:property value="id" />" style="display:none">
							<select id="channelNameValue<s:property value="id" />">
								<s:iterator value="channelList" >
									<s:if test="fk_channel_name==name">selected="selected"</s:if>
									<option <s:if test="fk_channel_name==name">selected="selected"</s:if> value="<s:property value="name"/>"><s:property value="label"/></option>
								</s:iterator>
							</select>
							...
							<div id="channelNameL<s:property value="id" />"><s:property value="fk_channel_name" /></div>
							
						</td>
						<td align="center"><s:property value="union_searchs" /></td>
						
						<td align="center"><s:property value="top_date" /></td>
						<td align="center">
							<s:if test='hasDirect'>
								有
							</s:if>
				...
				<td align="center">
							<s:if test="#session.user_permission.block_top_word != null">
							 ...
							 onclick="deleteAjax(<s:property value="id" />);return false" href="#" class="listbutton<s:if test='visible == 0'>, highlight</s:if>">
								...
								<a id="deleteForEverLink<s:property value="id" />"
								 onclick="deleteForEverAjax(<s:property value="id" />);return false" href="#" class="listbutton<s:if test='visible == 1'>, highlight</s:if>">
								永久删除 </a> &nbsp;<br />
							
						
						<a class="listbutton"
							href="<s:url action="TopWord_input">
								<s:param name="topWordId" value="id"/>
								<s:param name="channelName" value="channelName" />
								<s:param name="startDate" value="startDate" />
							</s:url>">
						<s:text name="topword.edit" /> </a>
				 ...
				 <tfoot>
					<tr>
						...
						<s:if test="pageInfo.hasPrevPage">
							<a href="<s:url action="TopWord_list"> 
        ...
        <div class="select"> <strong>Other Pages: </strong>
           <input type="hidden" id="pageurl" value="<s:url action="TopWord_list">          						
								<s:param name="channelName" value="channelName" />
								<s:param name="startDate" value="startDate" />
