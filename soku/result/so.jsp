...
for(int i=0;i<extLength;i++){
			...
			<%String littleBrief=StringUtils.trimToEmpty(proInfo.optString("brief")); if(StringUtils.trimToEmpty(proInfo.optString("brief")).length()>300)littleBrief=StringUtils.trimToEmpty(proInfo.optString("brief")).substring(0,300)+"...";  %>
				...
				<div class="gotoplay">
					<div class="btnplay_large"><a target="_blank" 
						onclick="sokuClickStat(this, true);" _log_type="2"    _log_ct="2" _log_pos="<%=i+1 %>"  _log_directpos="5">播放</a></div>
	...
	<%}}
		...
		JSONObject middInfo=programmeJson.optJSONObject("midd");
		JSONObject psInfo=programmeJson.optJSONObject("ProgrammeSite");
		if(proInfo==null || psInfo==null)continue;
	...
	<div class="tv">
		...
		<div class="detail">
			...
			<div class="T">
			        ...
			        for(int episodeIndex=0;episodeIndex<episodeArr.length()&& episodeIndex<43;episodeIndex++){
							...
							if (link!=null && !link.isEmpty()){
								out.print("<a href='"+StringUtils.trimToEmpty(link)+
									"' onclick='sokuClickStat(this, true);' _log_type='2' _log_ct='1' _log_pos="+(i+1)+"  _log_directpos='4' "+
						...
						<ul class="linkpanel panel_15">
								<%for(int jj=43;jj<episodeArr.length();jj++){
									...
									int orderId = episodeArr.getJSONObject(jj).optInt("orderId");
								...
								if(episodeArr.getJSONObject(jj)==null)continue;
									out.print("<li><a href='"+StringUtils.trimToEmpty(episodeArr.getJSONObject(jj).optString("url"))+
											"' onclick='sokuClickStat(this, true);' _log_type='2' _log_ct='1' _log_pos="+(i+1)+"  _log_directpos='4' "+
										">"+((orderStage<=0 || orderStage>2000)?orderId:orderStage)+"</a></li>");
								}%>																						
						</ul>
						<div class="clear"></div>
						</div>	
				...
				<div class="gotoplay">
					<div class="btnplay_large"><a target="_blank" 
					onclick="sokuClickStat(this, true);" _log_type="2"    _log_ct="1" _log_pos="<%=i+1 %>"  _log_directpos="5" >播放</a></div>
		...
		<div class="detail">
			...
			<div class="T">
					...
					if(episodeArr==null) continue;
						...
						%>
						   ...
						   <div class="aimtip">
						    <div class="close" onclick="this.parentNode.style.display = 'none';"></div>
						    <div class="arrow"></div>
   		     				<%=StringUtils.trimToEmpty(proInfo.optString("name"))%> 第<%=orderStage%>集
						</div>
						<%
						     }       
						     else{
						     ...
						     if (link!=null && !link.isEmpty()){
							     out.print("<a href='"+StringUtils.trimToEmpty(link)+
							    		 "' onclick='sokuClickStat(this, true);' _log_type='2' _log_ct='5' _log_pos="+(i+1)+"  _log_directpos='4' "+
									     	" target='_blank'>"+ ((orderStage<=0 || orderStage>2000)?orderId:orderStage) +"</a></li>");
						     }       
						     else{
							     //没链接地址显示为灰色
						...
						<ul class="linkpanel panel_15">
								...
								if(episodeArr.getJSONObject(jj)==null)continue;
									out.print("<li><a href='"+StringUtils.trimToEmpty(episodeArr.getJSONObject(jj).optString("url"))+
											"' onclick='sokuClickStat(this, true);' _log_type='2' _log_ct='5' _log_pos="+(i+1)+"  directpos='4' "+
										">"+((orderStage<=0 || orderStage>2000)?orderId:orderStage)+"</a></li>");
								}%>																						
						</ul>
						<div class="clear"></div>
				...
				<div class="gotoplay">
					...
					onclick="sokuClickStat(this, true);" _log_type="2"    _log_ct="5" _log_pos="<%=i+1 %>"  _log_directpos="5" >播放</a></div>
			...
			<div class="T">
				...
				if(!idList.contains(programmeId)){
					for(int ii=0;ii<siteArr.length();ii++){
						...
						int contentLength=0;
						int episodeIndex=0;
				        for(episodeIndex=0;episodeIndex<episodeArr.length()&& contentLength<14;episodeIndex++){
								if(episodeArr.getJSONObject(episodeIndex)==null)continue;
							
								 int orderStage = episodeArr.getJSONObject(episodeIndex).optInt("orderStage");
								 if(orderStage==0) continue;
								 ...
								 if (link!=null && !link.isEmpty()){
									 out.print("<a href='"+StringUtils.trimToEmpty(link)+
											 	...
											 	" onclick='sokuClickStat(this, true);' _log_type='2' _log_ct='3' _log_pos="+(i+1)+"  _log_directpos='4' "+
											 	">"+ orderStageStr +"</a></li>");
								 }       
								 else{
									 //没链接地址显示为灰色
							...
							%>											
								<%	if(episodeArr.length()>contentLength && contentLength==14){ %>									
								<li class="handle"><span>全部</span></li><%} %>
							</ul>
							<div class="clear"></div>
							<div class="panellocation">
							...
							<ul class="linkpanel panel_5">
									<%for(;episodeIndex<episodeArr.length();episodeIndex++){
									if(episodeArr.getJSONObject(episodeIndex)==null)continue;
										String title= StringUtils.trimToEmpty(episodeArr.getJSONObject(episodeIndex).optString("name"));
										...
										if(orderStage==0) continue;
										 ...
										 String link = StringUtils.trimToEmpty(episodeArr.getJSONObject(episodeIndex).optString("url"));
										 if (link!=null && !link.isEmpty() && orderStage>0){
											out.print("<li><a href='"+link+
											"' title='"+title+"'  target='_blank'"+
											" onclick='sokuClickStat(this, true);' _log_type='2' _log_ct='3' _log_pos="+(i+1)+"  _log_directpos='4' "+
											">"+orderStageStr+"</a></li>");
										 }else{
					...
					for(int ii=0;ii<siteArr.length();ii++){
						...
						int contentLength=0;
						int episodeIndex=0;
				        for(episodeIndex=0;episodeIndex<episodeArr.length()&& contentLength<9;episodeIndex++){
								if(episodeArr.getJSONObject(episodeIndex)==null)continue;
							...
							%>											
								<%	if(episodeArr.length()>contentLength && contentLength==9){ %>									
								<li class="handle"><span>全部</span></li><%} %>
							</ul>
							<div class="clear"></div>
							<div class="panellocation">
				...
				<div class="gotoplay">
					<div class="btnplay_large"><a target="_blank"
						onclick="sokuClickStat(this, true);" _log_type="2"    _log_ct="3" _log_pos="<%=i+1 %>"  _log_directpos="5" >播放</a></div>
	...
	<%
		...
		<ul class="p">
						...
						<li class="p_play"><div class="btnplay"><a  href="<%=personUrl %>"  target="_blank"
						 onclick="sokuClickStat(this, true);" _log_type="2"    _log_ct="6" _log_pos="<%=i+1 %>"  _log_directpos="5">播放</a></div></li>
		</ul>
...
<div class="items">
	<%
			...
			<ul class="v">
				...
				<li class="v_link"><a title="<%=title%>" target="_blank" href="<%=url%>" onclick="sokuClickStat(this, true);"  _log_type="3" _log_pos="<%=i+1 %>"></a></li>
				...
				<li class="v_title"><a title="<%=title%>" target="_blank" href="<%=url%>"  onclick="sokuClickStat(this, true);"  _log_type="3" _log_pos="<%=i+1 %>"  ><%=hltitle%></a></li>
				<li class="v_desc"> </li>
...
library=<%=hasDirect?1:0%>;result_count=<%=total%>;
</script>
