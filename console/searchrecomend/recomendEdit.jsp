...
<html>
	...
	<s:if test="0==t.id"><s:form action="searchrecomendstore.html" onsubmit="return checkForm(this);">
		<s:textfield id="keyword" name="t.keyword" label="关键词" required="true"/>
		<s:select name="t.recomendType" list="#{'video':'视频','folder':'专辑','user':'用户','bar':'看吧','all':'全部'}"/>
		<s:textfield id="queryCount" name="t.queryCount" label="搜索次数" maxLength="8"/>
		<s:textfield id="result" name="t.result" label="搜索结果" maxLength="8"/>
		<s:textfield onfocus="setday(this)" name="t.starttime" label="开始日期（空则日期取当天的前一天)(格式:yyyy-mm-dd)" />
		<s:textfield onfocus="setday(this)" name="t.endtime" label="结束日期（空则日期取当天的前一天)(格式:yyyy-mm-dd)" />
		<s:submit value="保 存"/>
		<s:reset value="取 消"/>
	</s:form>
	</s:if><s:else><s:form action="searchrecomendupdate.html" onsubmit="return checkForm(this);">
		<s:textfield id="keyword" name="t.keyword" label="关键词" required="true"/>
		<s:select name="t.recomendType" list="#{'video':'视频','folder':'专辑','user':'用户','bar':'看吧','all':'全部'}"/>
		<s:textfield id="queryCount" name="t.queryCount" label="搜索次数" maxLength="8" />
		<s:textfield id="result" name="t.result" label="搜索结果" maxLength="8" />
