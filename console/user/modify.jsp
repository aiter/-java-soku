...
<html>
	<head>
		...
		<script language="javascript">
			function checkForm(form)
			{
				...
				var newpwd=form.newPassword.value;
	...
	<s:form action="userupdate.html" onsubmit="return checkForm(this);">
			<s:textfield name="u.name" label="用户名" readonly="true"/>
			<FONT color="#FF0000"><s:property value="message"/></FONT>
			<s:textfield id ="trueName" name="u.trueName" label="真实姓名" required="true"/>
			<s:password id="password" name="u.password" label="旧密码" required="true"/>
			<s:password name="newPassword" label="新密码" required="true"/>
			<s:password name="confirmPassword" label="重复新密码" required="true"/>
			<s:textfield name="u.team" label="团队"/>
			<s:textfield onfocus="setday(this)" name="u.birth" label="生日" readonly="true"/>
			<s:radio name="u.sex" list="#{'0':'男','2':'女'}" label="性别"/>
			<s:submit value="保 存"/>
			<s:reset value="取 消"/>
</s:form>
<SCRIPT language=javascript>
		...
		*/
		...
		function meizzToday()  //Today Button
		{
			...
			//meizzSetDay(meizzTheYear,meizzTheMonth);
			if(outObject){
				outObject.value=meizzTheYear + "_" + meizzTheMonth + "_" + today;
			}
			closeLayer();
		}
		...
		if (outObject)
		{
			...
			return;}
			if ( n < 10){n = "0" + n;}
			outObject.value= yy + "_" + mm + "_" + n ; //注：在这里你可以输出改成你想要的格式
			closeLayer(); 
		}
